...
public final class ShipDto extends AbstractDto {
    ...
    public long getItemSakuteki(boolean useOriginalSakuteki) {
        List<ItemDto> items = this.getItem();
        long itemSakuteki = 0;
        for (int i = 0; i < 4; i++) {
            ItemDto item = items.get(i);
            if (item != null) {
                if ("9".equals(item.getTypeId3()) || "10".equals(item.getTypeId3())) { //9:艦上偵察機,10:水上偵察機
                    if (useOriginalSakuteki == false) { //偵察機：２倍
                        itemSakuteki += item.getSaku() * 2;
                    }
                    else { //装備オリジナル索敵値
                        itemSakuteki += item.getSaku();
                    }
                }
                else if ("11".equals(item.getTypeId3())) { //電探：１倍
                    itemSakuteki += item.getSaku();
                }
            }
        }
    ...
    public int getSakuteki25() {
        int sakuteki25 = 0;
        sakuteki25 = (int) Math.floor(this.getItemSakuteki(false)
                + Math.sqrt(this.sakuteki - this.getItemSakuteki(true)));
        return sakuteki25;
    }
